 ...
 * @param {string} rule 
    ...
    - Rule abbr. (number/string-length) -
        ...
        :Y  => 0:Y 
    - carry-in - 
        ^N ...      listen to the Nth emit carry-out,
                    it will be advanced again.
                    (cycle/sequentially only)
        -- example: {^2 2012:2023}/{^3 1-12}/{1:31}
 * @returns {function}
 */
export default function gen(rule, carry = () => { }) {
    if (rule === '') {
        ...
        return genSequentialFromFile(file, carry);
        ...
        if (Object.keys(charTable).includes(t)) {
            let [min, max] = parseRange(rule.slice(1), ':', -1, 256);
            if (min < 0) min = max;
            return genStringCyclic(t, min, max, carry);
        }
        const [min, max] = parseRange(rule, ':');
        return genCyclic(min, max, carry);
    } else {
        return () => `{${rule}}`;
    }
}
...
export function genSequentialFromFile(file, carry = () => { }) {
    const lines = fs.readFileSync(file, 'utf-8')
        .split('\n')
        .filter(Boolean)
        .map(line => line.trim());
    const nextIndex = genCyclic(0, lines.length - 1, carry);
...
export function genCyclic(min, max, carry = () => { }) {
    // let nexcurrentt = min;
    // return () => {
    //     const current = next;
    //     next += 1;
