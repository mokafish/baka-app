import { Buffer } from 'node:buffer';
import { Counter } from "./collection.js";
import { gen } from "./gen.js";

export class Token {
    constructor(type = 'raw', symbol = '') {
        this.type = type;
        this.symbol = symbol;
        this.pipes = [];
        this.value = '';
        this.eval = () => this.symbol
    }

    toString() {
        this.value = this.eval();
        return this.value;
    }

    splitPipe(sep = '|') {
        if (this.symbol.includes(sep)) {
            this.pipes = this.symbol.split(sep).map(p => p.trim());
            this.symbol = this.pipes.shift();
        }
        return this;
    }

}

export class VarToken extends Token {
    constructor(symbol) {
        super('var', symbol);
        // let parts = symbol.split('|')
        // this.expr = parts[0].trim();
        // this.encoding = parts[1] ? parts[1].trim() : '';
        this.splitPipe();
        this.eval = gen(this.symbol);
        // this.encoding = this.pipes[0];
    }

    toString() {
        let x = this.eval();
        this.value = this.encoding
            ? encoding(x, this.encoding)
            : String(x);

        return this.value;
    }
}

export class RefToken extends Token {
    constructor(target, context) {
        super('ref', target);
        this.target = target;
        this.context = context;
        this.eval = () => {
            if (this.context.table.has(this.target)) {
                return this.context.table.get(this.target).value;
            } else {
                throw new Error(`Reference "${this.target}" not found in context.`);
            }
        }
    }

    toString() {
        return this.eval();
    }
}

// class TokenList extends Array {
//     constructor(...args) {
//         super(...args);
//     }

//     appendRaw(content) {
//         this.push(new Token('raw', content));
//     }

//     appendVar(id, content) {
//         this.push(new VarToken(id, content));
//     }

//     toString() {
//         return render(this);
//     }
// }


export class AST {
    constructor() {
        this.table = new Map();
        this.cnt = new Counter();
    }

    addToken(type, content) {
        switch (type) {
            case 'raw':
                this.table.set('raw:' + this.cnt.inc('raw'), new Token(type, content));
                break;
            case 'var':
                this.table.set('var:' + this.cnt.inc('var'), new VarToken(content));
                break;
            case 'ref':
                this.table.set('ref:' + this.cnt.inc('ref'), new RefToken(content, this));
                break;
            default:
                throw new Error(`Unknown token type: ${type}`);
        }
    }

    toString() {
        return Array.from(this.table.values()).join('');
    }
}

export function compile(template = '', brackets = ['{', '}']) {
    const ast = new AST();
    const [left, right] = brackets;
    let startIndex = 0;
    let isInside = false;

    for (let i = 0; i < template.length; i++) {
        const char = template[i];
        if (!isInside && char === left) {
            // 遇到左括号，处理前面的原始内容
            if (i > startIndex) {
                ast.addToken('raw', template.slice(startIndex, i));
                // tokens.appendRaw(template.slice(startIndex, i));
            }
            isInside = true;
            startIndex = i + 1;
        } else if (isInside && char === right) {
            // 遇到右括号，处理表达式内容
            const expr = template.slice(startIndex, i).trim();
            if (expr.startsWith('=')) {
                // 处理变量引用表达式
                ast.addToken('ref', 'var:' + expr.slice(1).trim());
            } else {
                // 处理变量定义表达式
                ast.addToken('var', expr);
            }

            // tokens.appendVar(cnt.add('id'), template.slice(startIndex, i));
            isInside = false;
            startIndex = i + 1;
        }
    }

    // 处理最后剩余的原始内容
    if (!isInside && startIndex < template.length) {

        ast.addToken('raw', template.slice(startIndex));
    }

    return ast
}

export function render(ast) {
    return ast.toString();
}



export function pipeFunction(name, data) {
    switch (name) {
        case 'url':
            return encodeURIComponent(data);
        case 'hex':
            return Buffer.from(data).toString('hex');
        case 'HEX':
            return Buffer.from(data).toString('hex').toUpperCase();
        case 'base64':
            return Buffer.from(data).toString('base64');
        case 'base64url':
            return Buffer.from(data).toString('base64url');
        case 'base32':
            return Buffer.from(data).toString('base32');
        case 'BASE32':
            return Buffer.from(data).toString('base32').toUpperCase();
        default:
            throw new Error(`Unknown pipe function: ${name}`);
    }
}


/**
 * 
 * @param {number|string|Buffer} value 
 * @param {'hex'|'HEX'|'base64'} encoding 
 * @returns 
 */
export function encoding(value, encoding) {
    let buf = value;
    if (typeof value === 'number') {
        if (Number.isInteger(value)) {
            buf = Buffer.allocUnsafe(4);
            buf.writeInt32BE(value);
        } else {
            buf = Buffer.allocUnsafe(8);
            buf.writeDoubleBE(value);
        }
        // 去除前导零
        let i = 0;
        while (i < buf.length && buf[i] === 0) {
            i++;
        }
        if (i === buf.length) {
            // 全零，保留一个字节
            buf = Buffer.from([0]);
        } else {
            buf = buf.slice(i);
        }
    } else if (typeof value === 'string') {
        buf = Buffer.from(value);
    }
    else if (Buffer.isBuffer(value)) {
        buf = value;
    } else {
        throw new TypeError('Value must be a number, string or Buffer');
    }

    switch (encoding) {
        case 'hex':
            return buf.toString('hex');
        case 'HEX':
            return buf.toString('hex').toUpperCase();
        case 'base64':
            return buf.toString('base64');
        case 'base64url':
            return buf.toString('base64url');
        case 'base32':
            return buf.toString('base32');
        case 'BASE32':
            return buf.toString('base32').toUpperCase();
        default:
            return encodeURIComponent(buf.toString('utf8'));
    }
}

export default {
    Token,
    VarToken,
    compile,
    render
};