...
export class VarToken extends Token {
    constructor(symbol) {
        ...
        if (symbol.startsWith('^')) {
            ...
            this.low = symbol.slice(1, i).trim();
            this.symbol = symbol.slice(i + 1).trim();
        }

        this.splitPipe();
        this.carry_in = false;
        this.next = gen(this.symbol, this.emitCarryOut.bind(this));
        this.current = this.next();
        this.eval = () => {
            let x = this.current;
            if (this.low) {
                if (this.carry_in) {
                    console.log(`Carry in for ${this.symbol}`);
                    this.carry_in = false;
                    this.current = this.next();
                }
            } else {
        ...
        this.carryListeners = new Set();
    }

    emitCarryOut() {
        // this.flag = true;
        for (let f of this.carryListeners) {
            f();
        }
    }
}
...
export class AST {
    ...
    flush() {
        // 处理Token的相互关系
        for (let token of this.table.values()) {
            // 进位逻辑
            if (token.low) {
                const lowToken = this.table.get('var:' + token.low);
                if (lowToken) {
                    lowToken.carryListeners.add(() => token.carry_in = true);
                }
            }
        }
